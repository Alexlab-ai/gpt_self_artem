...
def build_tariffs_menu_markup() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⭐ Оформить Premium", callback_data="tariff_buy_premium")],
        [InlineKeyboardButton(text="👑 Оформить PRO", callback_data="tariff_buy_pro")],
        [InlineKeyboardButton(text="◀️ Назад", callback_data="root_menu")],
    ])
...
FAQ_SECTIONS = {
    "🪜 Работа по шагу": (
        "🪜 Работа по шагу\n\n"
        "Здесь ты проходишь программу 12 шагов.\n\n"
        "Как это работает:\n"
        "• Выбираешь шаг из списка (или продолжаешь текущий)\n"
        "• Читаешь вопрос и пишешь свой ответ\n"
        "• Ответ сохраняется автоматически\n"
        "• Переходишь к следующему вопросу\n\n"
        "Кнопки внутри шага:\n"
        "▶️ Продолжить — перейти к ответу на вопрос\n"
        "📋 Прогресс — посмотреть сколько сделано\n"
        "🧭 Помощь — если застрял или тяжело\n"
        "🧾 Описание шага — прочитать о чём этот шаг\n"
        "◀️ Назад — вернуться к выбору шага\n\n"
        "Если не можешь ответить — нажми 🧭 Помощь. "
        "Там три варианта: переформулировать вопрос, показать примеры или просто получить поддержку.\n\n"
        "Черновик сохраняется автоматически. Ты можешь выйти и вернуться — ответ будет на месте.\n\n"
        "Выбрать шаг или вопрос вручную можно в ⚙️ Настройки → 🔧 Настройки по шагу."
    ),
    "📖 Самоанализ": (
        "📖 Самоанализ (10 шаг)\n\n"
        "Ежедневная практика честности с самим собой.\n\n"
        "Как это работает:\n"
        "• 10 вопросов о прошедшем дне\n"
        "• Отвечаешь по порядку, можно пропустить\n"
        "• В конце — сохранение и возврат в меню\n\n"
        "Вопросы про: напряжение, старые модели поведения, "
        "честность, благодарность, духовные принципы.\n\n"
        "Рекомендуется делать каждый вечер — как чистка зубов, только для души."
    ),
    "📘 Чувства": (
        "📘 Колесо чувств\n\n"
        "Инструмент для распознавания своих эмоций.\n\n"
        "Как это работает:\n"
        "• Выбираешь категорию (радость, грусть, злость, страх...)\n"
        "• Внутри — список конкретных чувств\n"
        "• Нажимаешь — читаешь описание\n"
        "• Это помогает назвать то, что внутри\n\n"
        "Зачем: многие зависимые не умеют различать свои чувства. "
        "Мы привыкли всё называть «плохо» или «нормально». "
        "Колесо чувств помогает увидеть, что именно ты чувствуешь — "
        "а это первый шаг к тому, чтобы с этим справиться."
    ),
    "🙏 Благодарности": (
        "🙏 Благодарности\n\n"
        "Практика благодарности — часть выздоровления.\n\n"
        "Как это работает:\n"
        "• Нажми ➕ Добавить — напиши за что благодарен\n"
        "• Запись сохраняется с датой\n"
        "• 🗃️ История — посмотреть прошлые записи\n\n"
        "Зачем: благодарность меняет фокус внимания. "
        "Вместо «что плохого» — «что хорошего уже есть». "
        "Многие в программе пишут благодарности каждый день.\n\n"
        "Кто видит: только ты. Записи приватны."
    ),
    "📋 Прогресс": (
        "📋 Мой прогресс\n\n"
        "Здесь видно что ты уже сделал.\n\n"
        "• Список всех 12 шагов с количеством отвеченных вопросов\n"
        "• Можно выбрать любой шаг и посмотреть ответы\n"
        "• Можно переключиться на другой шаг\n\n"
        "Прогресс не теряется. Даже если выйдешь из бота — всё сохранено."
    ),
    "🧭 SOS / Помощь": (
        "🧭 SOS / Помощь\n\n"
        "Когда застрял или тяжело — жми 🧭 Помощь.\n\n"
        "Три варианта:\n"
        "💭 Не понял вопрос — бот переформулирует вопрос простыми словами\n"
        "🔍 Хочу примеры — бот покажет бытовые ситуации из жизни, "
        "чтобы помочь вспомнить своё\n"
        "🪫 Просто тяжело — бот просто будет рядом, без анализа и советов\n\n"
        "Помощь доступна всегда, даже на бесплатном тарифе.\n"
        "Это не слабость — это часть программы."
    ),
    "🗺 Навигация": (
        "🗺 Как устроен бот\n\n"
        "Главное меню:\n"
        "🪜 Работа по шагу — проходить 12 шагов\n"
        "📖 Самоанализ — ежедневные 10 вопросов\n"
        "📘 Чувства — колесо чувств\n"
        "🙏 Благодарности — записи благодарности\n"
        "⚙️ Настройки — выбор шага/вопроса, язык, напоминания\n"
        "🪪 Профиль — информация о себе\n\n"
        "Нижняя панель (всегда видна):\n"
        "📋 Меню — вернуться в главное меню\n"
        "💎 Тарифы — посмотреть планы подписки\n"
        "❓ Помощь — инструкция (ты сейчас здесь)\n\n"
        "Внутри шага:\n"
        "▶️ Продолжить — перейти к ответу\n"
        "📋 Прогресс — что сделано\n"
        "🧭 Помощь — SOS\n"
        "🧾 Описание — о чём этот шаг\n"
        "◀️ Назад — к списку шагов\n\n"
        "Кнопка ◀️ Назад всегда возвращает на шаг выше, не в главное меню."
    ),
    "📬 Контакты": (
        "📬 Контакты\n\n"
        "Обратная связь, вопросы, предложения:\n"
        "Telegram: @gptself\n\n"
        "Нашёл баг? Есть идея? Хочешь помочь проекту?\n"
        "Напиши — мы читаем каждое сообщение.\n\n"
        "GPT-SELF — бот для работы по программе 12 шагов.\n"
        "Создан зависимым для зависимых."
    ),
}
