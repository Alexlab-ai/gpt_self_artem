...
env_path = pathlib.Path(**file**).parent.parent.parent / “telegram.env”
...
“”“Markup for listing questions — groups of 15 if more than 15 total.”””
GROUP_SIZE = 15
total = len(questions)
buttons = []
...
if total <= GROUP_SIZE:
    for i in range(0, total, 3):
        ...
        for j in range(3):
            if i + j < total:
                q = questions[i + j]
                icon = "✅" if q.get("is_answered") else "⬜"
                row.append(InlineKeyboardButton(
                    text=f"{icon}{i+j+1}",
...
“”“Markup for a single group of questions (up to 15).”””
GROUP_SIZE = 15
